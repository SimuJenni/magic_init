...
UNSUPPORTED_LAYERS = ['Split', 'Reshape']
...
def gatherInputData(net, layer_id, bottom_data, top_name, fast=False, max_data=None):
    ...
    for i in range(OS):
        ...
        data = np.concatenate([i.swapaxes(0, 1).reshape((i.shape[1], -1)).T for i in
                               forward(net, layer_id, NIT, bottom_data, [top_name])[top_name]], axis=0)
...
def initializeLayer(net, layer_id, bottom_data, top_name, bias=0, type='elwise', max_data=None):
    ...
    if type == 'elwise':
        ...
        # Loop over groups
        for g in range(G):
            dg, Dg = d[g * (d.shape[0] // G):(g + 1) * (d.shape[0] // G)], D[:, g * (D.shape[1] // G):(g + 1) * (
            D.shape[1] // G):]
    ...
    for i, (n, l) in enumerate(zip(net._layer_names, net.layers)):
        ...
        if len(l.blobs) > 0:
            if np.sum(np.abs(l.blobs[0].data)) <= 1e-10:
                ...
                # Fill the parameters
                initializeLayer(net, i, {b: active_data[b] for b in net.bottom_names[n]}, net.top_names[n][0], bias,
...
def estimateHomogenety(net):
    ...
    for i, (n, l) in enumerate(zip(net._layer_names, net.layers)):
        # Run the network forward
        new_data1 = forward(net, i, 1, {b: [1 * d for d in active_data[b]] for b in net.bottom_names[n]},
                            net.top_names[n])
        new_data2 = forward(net, i, 1, {b: [2 * d for d in active_data[b]] for b in net.bottom_names[n]},
                            net.top_names[n])
    ...
    for it in range(10):
        ...
        for i, (n, l) in enumerate(zip(net._layer_names, net.layers)):
            ...
            if l.type in INPUT_LAYERS:
                ...
                active_data.update(
                    forward(net, i, NIT, {b: active_data[b] for b in net.bottom_names[n]}, net.top_names[n]))
            # Delete all unused data
            for k in list(active_data):
                if k not in last_used or last_used[k] == i:
                    del active_data[k]
...
def main():
    ...
    parser.add_argument('-t', '--type', default='elwise',
                        help='Type: elwise, pca, zca, kmeans, rand (random input patches). Add fast_ to speed up the initialization, but you might lose in precision.')
    ...
    if args.data is not None:
        ...
        f.flush()
        net.data, net.label = L.ImageData(source=f.name, batch_size=args.bs, new_width=model.input_dim[-1],
                                          new_height=model.input_dim[-1],
                                          transform_param=dict(mean_value=[104, 117, 123], scale=args.s), ntop=2)
    ...
    if any([np.abs(l.blobs[0].data).sum() < 1e-10 for l in n.layers if len(l.blobs) > 0]):
        print(
            [m for l, m in zip(n.layers, n._layer_names) if len(l.blobs) > 0 and np.abs(l.blobs[0].data).sum() < 1e-10])
